#!/usr/bin/env python3
"""Copy/sync events with matching locations from one Nextcloud calendar to another.

Every event in the source calendar whose LOCATION field contains one of the
configured location strings is mirrored into the target calendar. Running
the script again updates changed events and removes copies whose source
event was deleted or no longer matches - so source and target stay in sync
on every (e.g. daily) run. All settings come from a YAML config file.
"""
...
def load_config(raw: dict) -> Config:
    ...
    return Config(
        ...
        source_calendar=sync_cfg["source_calendar"],
...
def make_target_uid(source_uid: str) -> str:
...
def sync(config: Config) -> None:
    ...
    source_cal = find_calendar(principal, config.source_calendar)
    target_cal = find_or_create_calendar(principal, config.target_calendar)
    log.info("Source calendar: %s", config.source_calendar)
    ...
    log.info("Fetched %d calendar objects from source", len(source_objects))
